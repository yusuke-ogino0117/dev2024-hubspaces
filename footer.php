...
<?php } ?>
  ...
  <div id="footer_contents">
   <div class="inner">
      <p id="copyright-text"><span><?php _e('Copyright ', 'tcd-w'); ?></span>&copy; <a href="<?php echo esc_url(home_url('/')); ?>">Hub Works.inc</a>. All Rights Reserved.</p>
<?php
...
?>
  $('#header_slider').on('init', function(event, slick) {
    $(this).append('<div class="slick-counter"><span class="slide_current"></span> / <span class="total"></span></div>');
    $('.slide_current').text(slick.currentSlide + 1);
  ...
  .on('beforeChange', function(event, slick, currentSlide, nextSlide) {
    $('.slide_current').text(nextSlide + 1);
  });
...
<?php }elseif (is_singular($dp_options['introduce_slug']) && !empty($GLOBALS['introduce_slider'])) { ?>
    ...
    $('#introduce_slider').on('init', function(event, slick) {
      $(this).append('<div class="slick-counter"><span class="slide_current"></span> / <span class="total"></span></div>');
      $('.slide_current').text(slick.currentSlide + 1);
      $('.total').text(slick.slideCount);
    })
    .slick({
      ...
      if (ua_smt() == true) {
        echo "centerMode: false," ;
      } else {
        echo "centerMode: true," ;
        echo "centerPadding: '20%'," ;
      } ?>
      autoplaySpeed: <?php if ($dp_options['slider_time'] && is_numeric($dp_options['slider_time'])) { echo intval($dp_options['slider_time']); } else { echo 7000; } ?>
    })
    .on('beforeChange', function(event, slick, currentSlide, nextSlide) {
...
<?php if (is_post_type_archive('ranking') || is_tax('area_ranking') || strpos(esc_url($_SERVER['REQUEST_URI']), 'area_ranking')) { ?>
    ...
    $('.pref_list [data-id]').click(function(){
        if($(this).data('id')){
            var id = $(this).data('id');
            window.location.href = '<?php echo esc_url( home_url( '/' ) ); ?>area_ranking/' + id;
