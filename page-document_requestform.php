...
<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
 ...
 <div id="article">
  ...
  <h2>資料請求</h2>
  <form action="https://form.k3r.jp/hubworks/002" id="requestform" method="POST">
    <table>
      <tr>
        <th><span>必須</span> オフィス形態</th>
        <td>
          <input type="radio" name="f_item_customer_def18[]" value="レンタルオフィス・サービスオフィス(個室)" class="validate[required]"/>レンタルオフィス・サービスオフィス(個室)
      ...
      <tr>
        <th><span>必須</span>資料請求対象のオフィス名</th>
        <td>
          <input type="text" name="f_item_customer_def20" value="<?php if(isset($_GET['spacename'])){ echo $_GET['spacename'] ; } ?>" placeholder="(例)We Work新橋<" class="validate[required]" style="<?php if(isset($_GET['spacename'])){ echo 'pointer-events: none; color: #888; backgroud-color: #dbdcdd; border-width: 0px; box-shadow: none;' ; } ?>"/>
        </td>
      </tr>
      <tr>
        <th><span>必須</span> 入居予定人数</th>
        <td>
      ...
      <tr>
        <th><span>必須</span> 内覧希望時間帯・希望の賃料・お問い合わせ内容など</th>
        <td>
          <textarea name="f_item_customer_def16" rows="12" placeholder="内覧希望時間帯・希望の賃料・お問い合わせなど" class="validate[required]"></textarea>
        </td>
      </tr>
      <tr>
    ...
    <input type="hidden" name="red" value="https://hubspaces.jp/whitepaper-end/" />

    <!-- ↓↓↓オプション↓↓↓ -->
    <input type="hidden" name="tag" value="資料請求" />
